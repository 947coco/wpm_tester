...
def choisir_langue(langue):
    ...
    fichiertxt = f"{langue}.txt"
    mots = [mot for ligne in open(fichiertxt, "r") for mot in ligne.split()]
    fichiertxt.close()
    
choisir_langue("francais")
print(mots[::10]) # test ne marche pas
...
exit_x, exit_y = xy_pourcent(97, -1)
exit_w, exit_h = xy_pourcent(6, 6)
exit = pygame.Surface((exit_w, exit_h))
exit.fill(red)
window.blit(exit, (exit_x, exit_y))
...
while True:
    ...
    for evenement in pygame.event.get():
        if evenement.type == pygame.MOUSEBUTTONDOWN and evenement.button == 1:
            if exit_x <= evenement.pos[0] <= exit_x + exit_w and exit_y <= evenement.pos[1] <= exit_y + exit_h:
                pygame.quit(), quit()
        if evenement.type == pygame.MOUSEBUTTONDOWN and evenement.button == 1:
